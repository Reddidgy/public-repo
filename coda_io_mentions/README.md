...
- **MarkdownV2 Formatting**: Sends clearly structured Telegram messages using MarkdownV2 bolding and user-friendly aliases defined in `ENABLED_TGNAMES`.
...
| `ENABLED_TGNAMES` | Comma-separated list of Telegram usernames/display names corresponding to the emails. |

### User-Channel and Name Mapping Logic

The application maps monitored users to notification channels and their Telegram names by their **position** in the comma-separated lists. 

**Example:**
- `ENABLED_MENTIONS=alice@example.com, bob@example.com`
- `ENABLED_CHANNELS=111111, 222222`
- `ENABLED_TGNAMES=AliceInWonderland, BobBuilder`

Mentions of **Alice** will be sent to channel **111111** with the name **@AliceInWonderland**, while mentions of **Bob** will be sent to channel **222222** with the name **@BobBuilder**.
